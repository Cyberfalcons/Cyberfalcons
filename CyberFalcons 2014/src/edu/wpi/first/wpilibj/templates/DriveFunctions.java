...
public class DriveFunctions {
    ...
    public void setDriveLeft(double power) {
        if (power < -vm.DEADZONE || power > vm.DEADZONE) {
            if (!controlFlip) {
                ...
                driveLeft.set(power); // if controls are flipped the commanded left side is the physical right side
            }
        } else {
            if (!controlFlip) {
                driveLeft.set(0);
    ...
    public void setDriveRight(double power) {
        if (power < -vm.DEADZONE || power > vm.DEADZONE) {
            if (controlFlip) {
                driveRight.set(-power);// if controls are flipped the commanded left side is the physical left side
            } else {
